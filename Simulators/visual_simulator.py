# visual_simulator.py (Integrated with CubeTracker)

import sys
import os
import numpy as np
import time
from cube_simulator_full import CubeTracker
...
class StaticCubeViewer(ShowBase):
    def __init__(self):
        ...
        self.cube_tracker = CubeTracker()  # Track cube state
        self.piece_id_to_node = {}         # Maps piece IDs to Panda3D nodes
    ...
    def animate_move(self, move):
        ...
        # Determine affected piece IDs from current cube state
        affected_positions = self._get_affected_positions(move_upper)
        affected_piece_ids = [
            self.cube_tracker.piece_current_ids_at_positions[pos]
    ...
    def _get_affected_positions(self, move):
        """Get current positions affected by the move."""
        layer_map = {'U': 0, 'D': 2, 'L': 0, 'R': 2, 'F': 2, 'B': 0}
        axis_map = {'U': 1, 'D': 1, 'L': 2, 'R': 2, 'F': 0, 'B': 0}
        layer = layer_map[move]
        axis = axis_map[move]
        return [
            (i, j, k) for i in range(3) for j in range(3) for k in range(3)
            if (i, j, k)[axis] == layer and (i, j, k) != (1, 1, 1)
        ]
